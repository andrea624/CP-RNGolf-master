class Play extends Phaser.Scene {
    ...
    create() {
        ...
        //saving ball's start values
        this.ballStartX = width/2
        this.ballStartY = height - height/10
        this.ball = this.physics.add.sprite(this.ballStartX, this.ballStartY, 'ball')
        ...
        //changes x position of first wall to random
        wallA.setX(Phaser.Math.Between(0 + wallA.width/2, width - wallA.width/2))
        wallA.body.setImmovable(true)
        wallA.body.setVelocityX(100)
        wallA.body.setCollideWorldBounds(true).setBounce(1)

        let wallB = this.physics.add.sprite(0, height/2, 'wall')
        //changes x position of second wall to random
    ...
    //resets ball
    resetBall(){
        this.ball.setPosition(this.ballStartX, this.ballStartY); // Moves ball back to start
